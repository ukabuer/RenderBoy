...
public:
  TrackballControl(uint32_t width, uint32_t height)
...
private:
  enum class State { Rotate, Pan, Zoom, None };
  ...
  auto get_mouse_on_circle(int x, int y) -> Eigen::Vector2f {
    return {(static_cast<float>(x) - width * 0.5f) / (width * 0.5f),
            (height * 0.5f - static_cast<float>(y)) / (width * 0.5f)};
  }

  auto get_mouse_on_screen(int x, int y) -> Eigen::Vector2f {
